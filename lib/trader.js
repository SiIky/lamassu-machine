...
const Trader = function (clientCert, connectionInfo, dataPath, relayedModel) {
  ...
  this.request = options => {
    if (!options.form) options.json = true
    return _request(this.configVersion, globalOptions, options)
  }
  this.model = model
...
const arrayToFormData = array =>
  array.reduce(
    (form, elem, idx) => {
      form.append(idx, elem)
      return form
    },
    new FormData()
  )

Trader.prototype.updateIdCardPhotos = function updateIdCardPhotos (customerId, photos) {
  return this.request({
    path: `/customer/${customerId}/photos/idcarddata`,
    body: arrayToFormData(photos),
    method: 'PATCH',
    form: true,
  })
    .then(r => r.body)
